...
const VotePage = () => {
  ...
  const { data } = eventService.GetEvent(eventId as string, {
  ...
  return (
    <main>
      <CardHeader>
        <CardTitle className="text-2xl">{data?.voteTitle}</CardTitle>
        <CardDescription>
          ...
          {data?.candidates.map((v) => (
            <Card
              ...
              className={cn(
                ...
                v._id == selected.candidateId && "shadow-white/20 shadow-md",
              )}
              onClick={() =>
                setSelected((prev) => ({
                  candidateId: prev.candidateId == v._id ? null : v._id,
                }))
              }
            >
              <CardHeader className="flex flex-col gap-2">
                ...
                {selected.candidateId == v._id && (
                  <CheckIcon className="absolute right-5 top-0" />
                )}
              </CardHeader>
              <CardContent>
